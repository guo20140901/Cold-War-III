...
int last_day(){
	//成绩结算
	if(player_people>=25&&little_home==false){
		MessageBox(NULL,"解锁小国成就!","成就一",MB_OK);
		gold+=25;
		little_home=true;
	} 
	if(player_people>=50&&lb_home==false){
		MessageBox(NULL,"解锁中国成就!","成就二",MB_OK);
		gold+=50;
		lb_home=true;	
	}
	if(player_people>=75&&big_home==false){
	...
	player_silicon+=player_silicon*3;
	//事件发生
	if(player_hope<=25){
...
bool game_over(){
	...
	if(player_hope>150&&player_order>=150){
		...
		cout<<"#            #"<<endl;
		cout<<"##############"<<endl;
		MessageBox(NULL,"解锁胜利结局:亲民","结局三",MB_OK);
		return false;
	}  
}
...
int close(){
	std::string exeName = "冷战·生光 正式版 1.0.4.exe";  // 带空格的程序名
