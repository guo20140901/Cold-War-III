...
int home(){
		...
		printf("# 希望: %3d  # 5:建造食堂 [花费50金属,25聚合物] [暂无实际效果]  #\n",player_hope);
		cout<<"##############                    6:查询现有建筑                 #"<<endl;
		cout<<"# LG公司出品                                                     #"<<endl;
		cout<<"##################################################################"<<endl;
		cout<<"->>";
...
int close(){
	std::string exeName = "冷战·生光 正式版 1.0.8.exe";  // 带空格的程序名
...
void kaifazhe(){
	...
	cout<<"########################\n";
	cg=' ';
	while(cg!='1'&&cg!='2'&&cg!='3'&&cg!='4'){
		cg=_getch();
	}
	switch(cg){
		case '1':
			system("start https://www.luogu.com.cn/user/1797127");
			break;
		case '2':
			system("start http://oj.52steam.run/d/XCXX/user/222");
			break;
		case '3':
			system("start https://www.ccw.site/student/67acb1cf4b57da1266f87253");
			break;
		case '4':
			break;
	}
...
int play_again(){
	...
	while(play){
		...
		cout<<"-->";
		cg=' ';
		while(cg!='1'&&cg!='2'&&cg!='3'&&cg!='4'&&cg!='5'&&cg!='6'){
			cg=_getch();
		}
		switch(cg){
			case '1':
			...
			case '4':
				...
				break;
			case '5':
				system("start http://oj.52steam.run/d/XCXX/home/messages");
				break;
			case '6':
				play=false;
				break;
